...
socketTCP.connect((HOST_TCP, PORT_TCP)) # Double parentheses because connect has only one parameter.
...
while True: # The client can make n communications with the server, or until this connection is closed.
    ...
    socketTCP.sendall(str.encode(mensagemEnvioClient)) # Send message to server
    ...
    data = socketTCP.recv(1024) # Bytes
